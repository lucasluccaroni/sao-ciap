# DOCUMENTO MAESTRO DEFINITIVO V1.8: SISTEMA SAO BAR 2026
...
*   `insumo_compartido_id`: UUID (Opcional). FK autorreferencial (`Productos`) ON DELETE SET NULL. Apoya la relación de stock compartido (pizza -> bollo).
...
*   `unidades_utilizadas`: Integer. Unidades vendidas o insumos consumidos.
*   `unidades_regaladas`: Integer. Unidades entregadas mediante comandas con medio de pago 'Regalo'.
...
*   **Stock Teórico = Stock Inicial - Unidades Vendidas - Unidades Regaladas** (Leídos directamente de la foto en `Auditoria_Inventario`).
...
*   **Exclusión de Regalos**: Las comandas con medio de pago `'Regalo'` se omiten del cálculo de ventas comerciales de esta pestaña para evitar falsear las métricas de volumen cobrado y total general de facturación por producto.
...
*   **Formulario de Edición/Alta de Producto**:
    ...
    *   Asociación de Stock Compartido: Si se enlaza un insumo origen, los campos de Stock Inicial, Stock Actual y Stock Ideal se ocultan en el modal, mostrando una nota cian descriptiva. En la persistencia se graban silenciosamente en `0`.
...
2. Bloqueo de filas en productos o insumos compartidos vía `FOR UPDATE`.
...
4. **Auditoría Física**:
    - Carga de conteos en `Auditoria_Inventario` — **únicamente para productos con `controla_stock = true`**.
    - Persiste el conteo manual físico, las unidades utilizadas/vendidas, las unidades regaladas y el snapshot del `stock_inicial` de apertura.
    - **Tratamiento Inteligente de Insumos Compartidos**: Si un insumo posee dependientes asociados (ej. Bollo Pizza), el sistema lo clasifica como autocalculado (oculta el input manual de consumo y computa estáticamente la sumatoria total del consumo de las pizzas hijas en base a comandas), simplificando la tarea del administrador.
